...
def build_sections():
    ...
    my_button = None
    try:
        my_button = Button(rec_point_a, rec_point_b, LABEL)
    ...
    # shape
    instance_vars_section.add_items(Test("instance variable shape", type(my_button.button), Rectangle))
    # text
    instance_vars_section.add_items(Test("instance variable text", type(my_button.label), Text))
    ...
    for test in is_clicked_values:
        ...
        is_clicked_subsection.add_items(Test('click test', is_clicked, test[1], data=[f'click point: {test[0]}',
                                                                                      f'button dimensions: {(my_button.button.getP1(), my_button.button.getP2())}']))
    # color_button
    color_button_subsection = Section('color_button()')
    for color in get_colors():
        my_button.color_button(color)
        color_button_subsection.add_items(Test('color button', my_button.button.config["fill"], color))

    # set_label
    set_label_subsection = Section('set_label()')
    for label in get_colors():
        my_button.set_label(label)
        set_label_subsection.add_items(Test('set label', my_button.label.getText(), label))
